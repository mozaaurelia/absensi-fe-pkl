"use client";

import { useRouter } from "next/navigation";
import { FiClock, FiMapPin, FiShield } from "react-icons/fi";
import { useLanguage } from "@/context/LanguageContext";
import type { DashboardTodayAttendance, CurrentSchedule } from "@/lib/services/dashboard";

interface Props {
  todayAttendance?: DashboardTodayAttendance | null;
  currentSchedule?: CurrentSchedule | null;
}

export default function Attendance({ todayAttendance, currentSchedule }: Props) {
  const router = useRouter();
  const { t } = useLanguage();

  const hasCheckedIn = !!todayAttendance?.clock_in_time;
  const hasCheckedOut = !!todayAttendance?.clock_out_time;
  const isLate = todayAttendance?.status === "telat";

  const locationName = currentSchedule?.location_name || t("dashAttendance.headOffice");

  const badge = hasCheckedOut
    ? { text: t("dashAttendance.checkedOut"), color: "bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300" }
    : hasCheckedIn
      ? isLate
        ? { text: t("dashAttendance.late"), color: "bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-400" }
        : { text: t("dashAttendance.checkedIn"), color: "bg-green-100 dark:bg-green-500/20 text-green-700 dark:text-green-400" }
      : { text: t("dashAttendance.notCheckedIn"), color: "bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-400" };

  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-6 card-hover opacity-0 animate-fade-slide-up">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-blue-50 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400 flex items-center justify-center">
            <FiClock size={20} />
          </div>
          <div>
            <h3 className="font-bold text-gray-900 dark:text-gray-100">{t("dashAttendance.title")}</h3>
            <p className="text-xs text-gray-400 dark:text-gray-500">{t("dashAttendance.subtitle")}</p>
          </div>
        </div>
        <span className={`text-xs font-semibold px-3 py-1 rounded-full whitespace-nowrap ${badge.color}`}>
          {badge.text}
        </span>
      </div>

      <div className="grid grid-cols-2 gap-3 mb-4">
        <button
          onClick={() => router.push("/karyawan/attendance")}
          disabled={hasCheckedIn}
          className="bg-green-600 text-white font-semibold text-sm py-3 rounded-lg hover:bg-green-700 transition-all active:scale-95 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          <FiClock size={15} />
          {t("dashAttendance.checkIn")}
        </button>
        <button
          onClick={() => router.push("/karyawan/attendance")}
          disabled={!hasCheckedIn || hasCheckedOut}
          className={`font-semibold text-sm py-3 rounded-lg transition-all flex items-center justify-center gap-2 ${
            hasCheckedIn && !hasCheckedOut
              ? "bg-red-600 text-white hover:bg-red-700 active:scale-95"
              : "bg-gray-300 dark:bg-gray-700 text-gray-500 dark:text-gray-400 cursor-not-allowed"
          }`}
        >
          <FiClock size={15} />
          {t("dashAttendance.checkOut")}
        </button>
      </div>

      <div className="grid grid-cols-2 gap-3">
        <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg px-4 py-3 flex items-center gap-3">
          <FiMapPin size={16} className="text-gray-400 shrink-0" />
          <div>
            <p className="text-xs text-gray-400 dark:text-gray-500 mb-0.5">{t("dashAttendance.location")}</p>
            <p className="text-sm font-semibold text-gray-800 dark:text-gray-100 truncate">{locationName}</p>
          </div>
        </div>
        <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg px-4 py-3 flex items-center gap-3">
          <FiShield size={16} className="text-gray-400 shrink-0" />
          <div>
            <p className="text-xs text-gray-400 dark:text-gray-500 mb-0.5">{t("dashAttendance.verification")}</p>
            <p className="text-sm font-semibold text-gray-800 dark:text-gray-100">{t("dashAttendance.gpsValid")}</p>
          </div>
        </div>
      </div>
    </div>
  );
}
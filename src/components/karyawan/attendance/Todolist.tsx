...
export default function Todolist() {
      ...
      try {
        ...
        if (rows.length === 0) {
          ...
          try {
            ...
            if (prevItems.length > 0 && !cancelled) {
              ...
              if (created.length > 0) rows = created;
            }
          } catch {
        ...
        if (rows.length === 0 && !cancelled) {
          const defaults = [
            ...
            t("todolist.defaultTask3"),
          ];
          const created: TaskItem[] = [];
          for (const title of defaults) {
            try {
              created.push(await createTask(title, dateKey));
            } catch {
              // keep going on individual failures
            }
          }
  ...
  const addItem = async () => {
    ...
    setInput("");
    try {
      const created = await createTask(title, dateKey);
  ...
  const toggleItem = async (id: string) => {
    ...
    if (!target || busyIds.includes(id)) return;
    markBusy(id, true);
    ...
    <div className="bg-white/80 dark:bg-gray-800/80 rounded-[28px] border border-slate-200/80 dark:border-gray-700 p-5 shadow-[0_12px_28px_rgba(15,23,42,0.06)] h-full flex flex-col backdrop-blur-sm">
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <button
          onClick={addItem}
          disabled={isLoading}
          className="w-10 h-10 rounded-2xl bg-[#1E3A5F] text-white flex items-center justify-center hover:brightness-110 transition-colors shadow-sm shrink-0 disabled:opacity-60"
      ...
      <div className="space-y-3 flex-1 overflow-y-auto max-h-80 pr-1">
        {isLoading ? (
          <div className="rounded-2xl border border-dashed border-slate-200 dark:border-gray-600 bg-slate-50 dark:bg-gray-700/50 px-4 py-10 text-center">
            <p className="text-sm text-slate-400 dark:text-gray-400 animate-pulse">...</p>
          </div>
        ) : items.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-slate-200 dark:border-gray-600 bg-slate-50 dark:bg-gray-700/50 px-4 py-10 text-center">

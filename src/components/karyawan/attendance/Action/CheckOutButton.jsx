export default function CheckOutButton({ onClick, disabled = false }) {
  return (
    ...
    >
      <span className="relative w-24 h-24 rounded-full bg-gradient-to-br from-red-500 to-rose-600 flex items-center justify-center text-white shadow-lg shadow-red-900/20 transition-transform group-hover:scale-105 group-active:scale-95">
        {!disabled && (
      ...
      <span className="text-sm font-semibold text-gray-800">
        {disabled ? "Sudah Absen Pulang" : "Absen Pulang"}
      </span>
    </button>
  );
}
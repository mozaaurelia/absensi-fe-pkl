...
export default function AtasanOvertimePage() {
  const { t } = useLanguage();
  const [requests, setRequests] = useState<OvertimeTeamRequest[]>([]);
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await getTeamOvertimeRequests();
  ...
  return <AtasanAccessGuard><AtasanLayout><AtasanHeader />{loading ? <div className="h-72 bg-white dark:bg-gray-800 rounded-2xl animate-pulse" /> : error ? <PageError message={error} onRetry={loadData} /> : <OvertimeApproval requests={requests} onProcessed={loadData} />}</AtasanLayout></AtasanAccessGuard>;
}
